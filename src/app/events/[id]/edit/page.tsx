...
export default function EditEventPage() {
  ...
  useEffect(() => {
    async function fetchEvent() {
      setLoading(true);
      try {
        const res = await fetch(`http://localhost:8090/events/${eventId}`);
        ...
        setForm({
          ...
          maxCustomers: data.maxCustomers?.toString() || "",
          ...
          organizerId: data.organizer?.id?.toString() || "",
          categories: data.categories?.map((cat: any) => cat.key) || [],
        });
      } catch (err: any) {
  ...
  return (
    <div className="max-w-2xl mx-auto p-8">
      <h1 className="text-2xl font-bold mb-6">Éditer l'événement</h1>
      <form onSubmit={handleSubmit} className="space-y-4 bg-white p-6 rounded shadow">
        ...
        <div>
          ...
          <select name="organizerId" value={form.organizerId} onChange={handleChange} required className="w-full border p-2 rounded">
            <option value="">Sélectionner...</option>
            {users?.map(user => (
        ...
        <div className="flex gap-4">
          <div className="flex items-center gap-2">
            ...
            <label htmlFor="isTrending" className="font-medium">Tendance</label>
          </div>
          <div className="flex items-center gap-2">
            <input type="checkbox" name="isFirstEdition" checked={form.isFirstEdition} onChange={handleChange} id="isFirstEdition" />
            <label htmlFor="isFirstEdition" className="font-medium">Première édition</label>
          </div>
          <div className="flex-1">
